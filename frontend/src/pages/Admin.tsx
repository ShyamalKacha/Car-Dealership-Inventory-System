...
export function Admin() {
  ...
  const handleAdd = async (e: FormEvent) => {
    ...
    if (!form.make || !form.model || !form.price) {
      setFormError("Make, model, and price are required");
      return;
    }
    setSubmitting(true);
    try {
  ...
  const handleEdit = async (e: FormEvent) => {
    ...
    try {
      ...
      setFormError(err.response?.data?.detail || "Failed to update vehicle");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleRestock = async () => {
    ...
    if (!qty || qty < 1) return;
    try {
      await api.post(`/api/vehicles/${restockId}/restock`, { quantity: qty });
      setRestockId(null);
      setRestockQty("1");
      fetchVehicles();
    } catch (err: any) {
  ...
  return (
    <div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                ...
                {vehicles.map((v) => (
                  <tr key={v.id} className="border-b border-[var(--color-border)] last:border-0 hover:bg-[var(--color-bg-secondary)]/50">
                    ...
                    <td className="px-4 py-3 text-right">${Number(v.price).toLocaleString()}</td>
      ...
      <Modal open={deleteId !== null} onClose={() => setDeleteId(null)} title="Delete Vehicle">
        <p className="mb-4 text-sm text-[var(--color-text-secondary)]">Are you sure you want to delete this vehicle? This action cannot be undone.</p>
...
// ---------------------------------------------------------------------------
// Shared vehicle form (used in both Add and Edit modals)
// ---------------------------------------------------------------------------
function VehicleForm({
  form, setForm, error, onSubmit, submitting, submitLabel, onCancel,
}: {
  form: FormData;
  ...
  return (
    <form onSubmit={onSubmit} className="flex flex-col gap-4">
      ...
      <div>
        ...
        <select
          className="w-full rounded-lg border border-[var(--color-border)] bg-[var(--color-bg-primary)] px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[var(--color-accent)]"
          value={form.category}
